...
function PlayerModal({ player, matches, goals, totalMatches, onClose }: {
  player: Player; matches: Match[]; goals: Goal[]; totalMatches: number; onClose: () => void;
}) {
  // Only show matches where the player actually attended (P=1), i.e. in lineup or bench
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg bg-[#0A1A3E] border border-white/15 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-start justify-between px-5 py-4 border-b border-white/10 bg-[#11296B]/50">
          <div>
          ...
          <div className="flex items-start gap-3">
            {[
              ...
              <div key={s.label} className="text-center">
                ...
                <div className="text-white/40 text-[9px] uppercase tracking-tight">{s.label}</div>
              </div>
            ))}
            <button onClick={onClose} className="text-white/40 hover:text-white text-xl leading-none ml-1 cursor-pointer mt-1">✕</button>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 bg-[#11296B]/30">
                <th className="px-4 py-2 text-left text-white/40 text-[10px] uppercase tracking-wider">Partido</th>
                <th className="px-4 py-2 text-left text-white/40 text-[10px] uppercase tracking-wider">Rival</th>
                <th className="px-4 py-2 text-center text-white/40 text-[10px] uppercase tracking-wider">Rol</th>
                <th className="px-4 py-2 text-center text-white/40 text-[10px] uppercase tracking-wider">⚽</th>
              </tr>
            </thead>
            <tbody>
              {attended.map(m => {
                ...
                return (
                  <tr key={m.id} className="border-b border-white/5 hover:bg-[#11296B]/20">
                    ...
                    <td className="px-4 py-2.5 text-white/80 text-xs">{m.rival}</td>
